...
public class IntegrationTest {
    ...
    void ensureThatUserAPICallReturnStatusCode200() throws Exception {
        ...
        HttpRequest request = HttpRequest.newBuilder().uri(URI.create("http://localhost:8080/api/admin/flight/all")).build();
    ...
    @Test
    @DisplayName ("Ensure that the JSON for flight contains a flight name")
    void ensureJsonContainsBody() throws Exception {
        HttpClient client = HttpClient.newBuilder().build();
        HttpRequest request = HttpRequest.newBuilder().uri(URI.create("http://localhost:8080/api/admin/flight/all")).build();
        HttpResponse<String> response = client.send(request, BodyHandlers.ofString());
        String body = response.body();
        System.out.println(body);
        assertTrue(body.contains("fromCity\":\"Dnipro\""));
